...
/*        *//**
            ...
            User.findOne({
                ...
                user.save(function(err) {
                    ...
                    /*req.logIn(user, function(err) {
                        if (err) return next(err);
                        return  res.json({'code': '200', 'msg': "Password updated succesfully!"});
                    });*/
                    res.json({'code': '200', 'msg': "Password updated succesfully!"});
                });
            });
        },
        ...
        changePassword: function(req, res, next) {
          ...
          if (user.hashPassword(req.body.currentPassword) !== user.hashed_password) {
            return res.status(400).json({code: '0020',param: 'currentpassword' ,msg: 'Your password does not match'});
          }
          req.assert('password', 'Password must be between 8-20 characters long').len(8, 20);
          ...
          user.password = req.body.password;
          user.save(function(err) {
            return res.json({code: '1007',param: 'passwordSuccess' ,msg: 'Your password changed successfully.'});
          });
        },
        ...
        forgotpassword: function(req, res, next) {
            ...
            function(err, user) {
                ...
                if (err) {
                    response.message = 'Username/Password does not match correctly. Please try again';
