...
module.exports = function (RoleCtrl) {
    ...
    return {
         ...
         */
         update: function (req, res) {
            ...
            role.save(function (err) {
                ...
                FeatureRoleModel.remove({role: role._id, $softRemove: false}, function (err,removed) {
                	if(err){
                		res.json(err);
                	}
                	else {	
        ...
        //filtered roles for team
       filteredRoles: function (req, res) {

            RoleModel.find({ $or: [ {'name': 'FrontOffice'},{'name' : 'BackOffice'}]}).populate('features', 'name').sort({ name: 'asc' }).exec(function (err, roles) {
                if (err) {
                    return res.status(400).json({
                        error: 'Cannot list the Roles'
                    });
